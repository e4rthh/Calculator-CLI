...
tokenize(const std::string
             &expr) { // function stdvector string for outputing as a vector of
                      // string,t he const std::string& expr for const
                      // std::string it just says i wont do anything to the
                      // string and we input the variable as expr
  std::vector<std::string> tokens; // tokens vector string
  std::string current;             // string for current to pop

  for (size_t i = 0;
       i < expr.size();) { // loop for charecter in expr (aka string)
    char c = expr[i]; // changed to this because we can now check in the strings
                      // instead of by letters like sqrt(4) wil now be [sqrt(4)]
                      // in which we can easily check using to compare string
    ...
    if (!current.empty()) {      // if a symbol
      ...
      current.clear();
      continue; // clear the string using continue to avoid having 4 be checked
                // again in the if below
    }
    ...
    if (c != ' ') {
      tokens.push_back(std::string(
          1, c)); // push the symbol in 1, c stands for string of 1 charecter
      i++;
    }
  }
...
std::vector<std::string> shuting_yard(const std::vector<std::string> &tokens) {
  ...
  for (const auto &t : tokens) {
    ...
    if (std::isdigit(t[0])) {
    ...
    else if (t == ")") { // put the stuff in parenthesses to the output  by
                         // checking for ) then loops every thing before (
      ...
      if (isltr(t)) { // check if its left to right operation if it is then just
                      // loop normally
        while (!ops.empty() && ops.top() != "(" &&
               precedence(ops.top()) >=
                   precedence(t)) { // check if its empty and if its a higher
                                    // weighted operation
          output.push_back(ops.top()); // if it is then push to output
          ops.pop();
        }
      } else { // because of ltr condition if its both expo then it just gets
               // put into the stack
        while (!ops.empty() && // reason being ex 2^3^4+2. the parenthesses gets
                               // push to the stack and + so both of them go to
                               // the output but the output os now 2 3 4 ^ ^ +
                               // and now its correct order instead of 2 3 ^ 4 ^
               ops.top() != "(" && precedence(ops.top()) > precedence(t)) {
          output.push_back(ops.top());
          ops.pop();
        }
      }
...
double evaluvation(const std::vector<std::string> &tokens) {
  ...
  for (const auto &t : tokens) {
    if (std::isdigit(static_cast<unsigned char>(
            t[0]))) { // is digit can only check for unsigned char so we just
                      // change its type to that
